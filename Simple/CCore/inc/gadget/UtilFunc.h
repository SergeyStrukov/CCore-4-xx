...
template <class T>
constexpr T Min_cast(T a,AnyType auto ... args)
 {
  return ( OpAddHelper<T,Min>(a) + ... + OpAddHelper<T,Min>(args) ).val;
 }

template <class T>
constexpr T Max_cast(T a,AnyType auto ... args)
 {
...
char UpperHexDigit(UIntType auto digit) { return "0123456789ABCDEF"[digit]; }

char LowerHexDigit(UIntType auto digit) { return "0123456789abcdef"[digit]; }

char DecDigit(UIntType auto digit) { return char( '0'+digit ); }
