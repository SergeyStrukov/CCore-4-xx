...
template <class H>
concept ArrayHeaderType = PODType<H> && requires(H obj,ulen len)
 {
  Ground<ulen H::*>( &H::len );

  Ground<ulen H::*>( &H::maxlen );
...
concept BuilderType = requires(Meta::ToConst<Builder> &cobj)
 {
  Ground<ulen>( cobj.getLen() );
