...
class DynArrayBase : NoCopy
 {
   ...
   DynArrayBase() noexcept
    {
     ptr=DefaultDynArrayHeader::Get();
    }

   explicit DynArrayBase(DynArrayHeader *&ptr_) noexcept
    {
...
class DynArray : DynArrayBase<T,Algo>
 {
  ...
  public:
   ...
   DynArray(DoCreate dotype,CreatorType<T> auto creator)
    : DynArrayBase<T,Algo>(dotype.getMaxLen())
    {
     Base::Extend(ptr,dotype.getLen(),creator);
    }

   DynArray(DoBuildType,BuilderType<T> auto builder)
   ...
   DynArray<T,Algo> & reserve(ulen extra_len)
    {
     provide(extra_len);
   ...
   void objSwap(DynArray<T,Algo> &obj) noexcept
    {
     Swap(ptr,obj.ptr);
    }

   explicit DynArray(ToMoveCtor<DynArray<T,Algo> > obj) noexcept
    : DynArrayBase<T,Algo>(obj->ptr)
    {
    }
 };
