...
struct PrintBy
 {
  ...
  void print(PrinterType auto &out) const { func(out); }
 };
...
concept ProxyPrintType = requires(PrintBase &out,Proxy &obj)
 {
  obj.print(out);
 } ;
...
concept ProxyOptPrintType = requires(PrintBase &out,Proxy &obj,Opt &opt)
 {
  obj.print(out,opt);
 } ;
...
struct PrintAdapter<T>
 {
  ...
  static void Print(PrinterType auto &out,const char *ptr,const char *lim,const T &t)
   {
    PrintDesc(out,ptr,lim,GetTextDesc(t));
   }

  static void Print(PrinterType auto &out,const AnyType auto &opt,const T &t)
   {
    PrintDesc(out,opt,GetTextDesc(t));
   }

  static auto Print(PrinterType auto &out,const T &t) -> decltype( PrintDesc(out,GetTextDesc(t)) )
   {
...
struct PrintAdapter<T>
 {
  ...
  static void Print(PrinterType auto &out,const char *,const char *,const T &t)
   {
    ProxyType proxy(t);
  ...
  static void Print(PrinterType auto &out,const AnyType auto &,const T &t)
   {
    ProxyType proxy(t);
  ...
  static void Print(PrinterType auto &out,const T &t) requires ( ProxyPrintType<ProxyType> )
   {
    ProxyType proxy(t);
...
struct PrintAdapter<T>
 {
  ...
  static void Print(PrinterType auto &out,const OptType &opt,const T &t)
   {
    ProxyType proxy(t);
  ...
  static void Print(PrinterType auto &out,const T &t) requires ( ProxyOptPrintType<ProxyType,OptType> )
   {
    OptType opt{};
    ProxyType proxy(t);
...
template <class T>
concept PrintableType2 = requires(PrintBase &out,const T &t)
 {
...
template <class T>
concept PrintableType = IsPrintableType< Meta::UnConst< Meta::UnRef<T> > > ;
