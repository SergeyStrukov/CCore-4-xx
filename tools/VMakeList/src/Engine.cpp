...
template <class Func>
void Engine::printBy(PrinterType auto &,Func)
 {
  Printf(Exception,"App : incompatible variable value");
 }

template <FuncArgType<PrintBase &> Func>
void Engine::printBy(PrinterType auto &out,Func func)
 {
  func(out);
 }

void Engine::printVarExt(PrinterType auto &,StrLen str)
 {
  Printf(Exception,"App : unknown variable #.q;",str);
 }

template <class FuncSrc,class FuncDst>
void Engine::printVarExt(PrinterType auto &out,StrLen str,FuncSrc psrc,FuncDst pdst)
 {
  if( str.equal("SRC"_c) )
    {
     printBy(out,psrc);
...
void Engine::printVarExt(PrinterType auto &out,StrLen str,FuncSrc psrc,FuncDst pdst,FuncObj pobj)
 {
  if( str.equal("SRC"_c) )
    {
     printBy(out,psrc);
...
template <class ... TT>
void Engine::printVar(PrinterType auto &out,StrLen str,TT ... tt)
 {
  LockUse lock(level);
...
template <class ... TT>
void Engine::printText(PrinterType auto &out,StrLen str,TT ... tt)
 {
  while( +str )
    {
     ScanStr scan(str,'#');
...
template <class ... TT>
void Engine::printDDLString(PrinterType auto &out,StrLen str,TT ... tt)
 {
  if( !str )
    {
     Putobj(out,"''");
...
void Engine::printDefText(PrinterType auto &out,StrLen name,StrLen str)
 {
...
void Engine::printSubList(PrinterType auto &out,PrintFirst &stem,List list,TT ... tt)
 {
  LockUse lock(level);
...
template <class Func>
void Engine::printBy(PrinterType auto &out,PrintFirst &stem,Func func)
 {
  func(out,stem);
 }
...
template <class FuncSrc,class FuncDst>
bool Engine::printSubExt(PrinterType auto &out,PrintFirst &stem,StrLen str,FuncSrc psrc,FuncDst pdst)
 {
  if( str.equal("SRC"_c) )
    {
...
template <class FuncSrc,class FuncDst,class FuncObj>
bool Engine::printSubExt(PrinterType auto &out,PrintFirst &stem,StrLen str,FuncSrc psrc,FuncDst pdst,FuncObj pobj)
 {
  if( str.equal("SRC"_c) )
    {
     printBy(out,stem,psrc);
...
template <class ... TT>
bool Engine::printSub(PrinterType auto &out,PrintFirst &stem,StrLen str,TT ... tt)
 {
...
template <class List,class ... TT>
void Engine::printList(PrinterType auto &out,List list,TT ... tt)
 {
  out.put('{');
...
void Engine::genProj(PrinterType auto &out,FileList &cpp_list,FileList &asm_list)
 {
...
void Engine::genPrep(PrinterType auto &out,FileList &cpp_list,FileList &)
 {
  printDefText(out,"CC"_c,tools->CC);
