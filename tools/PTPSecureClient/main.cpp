...
void Executor::cmd_type(StrLen arg)
 {
  ...
  while( file.more() )
    {
     Putobj(Con,StrLen(Mutate<const char>(file.pump())));
    }
 }
...
void Executor::cmd_put(StrLen file_name,StrLen arg)
 {
  ...
  PutAsyncBinaryFile out("host",path);

  while( file.more() ) out.put(file.pump());
 }
     ...
     switch( ToBasicChar(ch) )
       {
        ...
        default:
         {
          StrLen sym=SymbolRange(ch);

          ulen new_len=arg_len+sym.len;

          if( new_len<=MaxArgLen && CharIsPrintable(ch) )
            {
             sym.copyTo(arg+arg_len);
