...
template <class File>
void CopyFile(File &file,SaveDevType auto &dev)
 {
  constexpr ulen BufLen = 64_KByte ;
...
void Pack(StrLen dir_name,StrLen file_name)
 {
  ...
  header.names_off=AddMul(HeaderSaveLoadLen,EntrySaveLoadLen,list.len);
...
void CreateDir(FileSystem &fs,StrLen path)
 {
...
void CreatePath(FileSystem &fs,StrLen path)
 {
  WalkPath(path, [&fs,path] (StrLen sub) { if( sub.len<path.len ) CreateDir(fs,sub); } );
 }
  ...
  if( cmd.equal("-a"_c) )
    {
     ...
     if( PathIsBase(dir_name) )
       {
        Printf(Exception,"App::Main(...) : #.q; is not allowed as a directory name",dir_name);
       }
