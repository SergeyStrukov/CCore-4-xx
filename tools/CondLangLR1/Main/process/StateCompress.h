  ...
  struct StateDesc : NoCopy
   {
    ...
    void print(PrinterType auto &out,ExtLangOpt opt) const
     {
      Printf(out,"#;) #;\n",index,prop_index);
...
struct State
 {
  ...
  bool operator == (State obj) const noexcept { return getExtIndex() == obj.getExtIndex() ; }

  CmpResult operator <=> (State obj) const noexcept { return Cmp(getExtIndex(),obj.getExtIndex()); }
 };
...
class StateCompress : public StateCompressBase
 {
   ...
   struct ToCmp
    {
     ...
     bool operator == (const ToCmp &obj) const noexcept
      {
       return ( (*this) <=> obj ) == 0 ;
      }

     CmpResult operator <=> (const ToCmp &obj) const noexcept
      {
  ...
  public:
   ...
   void applyForStates(FuncInitArgType<State> auto func_init) const
    {
