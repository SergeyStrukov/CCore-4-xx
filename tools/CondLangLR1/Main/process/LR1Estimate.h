...
class LR1Estimate
 {
   ...
   struct ExtRuleSet
    {
     ...
     bool operator == (const ExtRuleSet &obj) const noexcept
      {
       return shift==obj.shift && rules==obj.rules ;
      }

     CmpResult operator <=> (const ExtRuleSet &obj) const noexcept
      {
  ...
  public:
   ...
   bool operator == (const LR1Estimate &obj) const noexcept
    {
     return empty==obj.empty && null==obj.null && alpha==obj.alpha && beta==obj.beta ;
    }

   CmpResult operator <=> (const LR1Estimate &obj) const noexcept
    {
