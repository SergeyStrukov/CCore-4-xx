...
struct NameIdNode : NoCopy
 {
  ...
  void init(ExtContext ctx,const Token &token)
   {
    posname.init(ctx.getFileId(),token);
  ...
  void init_qname(ExtContext ctx,const Token &token)
   {
    posname.init_qname(ctx.getFileId(),token);
  ...
  StrLen getStr() const { return posname.str; }
  ...
  CmpResult operator <=> (From obj) const
   {
    ...
    for(ScopeNode *a=scope,*b=obj.scope; a ;a=a->prev,b=b->prev)
      {
       if( auto ret=Cmp(a->name.name_id,b->name.name_id) ) return ret;
      }
