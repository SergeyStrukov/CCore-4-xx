...
using CoordFunc = SIntFunc<Coord> ;
...
template <class ... TT>
Coord AddSize(Coord a,TT ... tt) requires ( sizeof ... (TT) >= 2 )
 {
...
Coord MulSize(UIntType auto count,Coord x)
 {
  if( x>0 && count>CoordFunc::UInt(CoordFunc::MaxPositive/x) ) GuardSizeOverflow("CCore::Video::MulSize");
...
Coord CountToCoord(UIntType auto count) { return Coord(count); }
...
struct BasePoint
 {
  ...
  T supX(Int sx) const { return T(Max(x,sx),y); }

  T supY(Int sy) const { return T(x,Max(y,sy)); }

  T sup(Int sx,Int sy) const { return T(Max(x,sx),Max(y,sy)); }

  T sup(T s) const { return sup(s.x,s.y); }

  T infX(Int sx) const { return T(Min(x,sx),y); }

  T infY(Int sy) const { return T(x,Min(y,sy)); }

  T inf(Int sx,Int sy) const { return T(Min(x,sx),Min(y,sy)); }
...
template <class ... TT>
Point StackXSize(Point a,TT ... tt) requires ( sizeof ... (TT) >= 2 )
 {
  using Helper = OpAddHelper<Point,StackXSize> ;

  return ( Helper(a) + ... + Helper(tt) ).val;
 }

template <class ... TT>
Point StackYSize(Point a,TT ... tt) requires ( sizeof ... (TT) >= 2 )
 {
  using Helper = OpAddHelper<Point,StackYSize> ;

  return ( Helper(a) + ... + Helper(tt) ).val;
 }

template <class ... TT>
Point StackXSize_guarded(Point a,TT ... tt) requires ( sizeof ... (TT) >= 2 )
 {
  using Helper = OpAddHelper<Point,StackXSize_guarded> ;

  return ( Helper(a) + ... + Helper(tt) ).val;
 }

template <class ... TT>
Point StackYSize_guarded(Point a,TT ... tt) requires ( sizeof ... (TT) >= 2 )
 {
  using Helper = OpAddHelper<Point,StackYSize_guarded> ;
