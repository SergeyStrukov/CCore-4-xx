...
struct ConId
 {
  ...
  friend bool operator == (const ConId &a,const ConId &b) = default ;
  ...
  static constexpr auto Fold()
   {
    return Ret(&T::slot,&T::number,&T::clock);
   }
 };
...
struct OpenInput // + uint8 name[len];
 {
  ...
  static constexpr auto Fold()
   {
    return Ret(&T::write_timeout_msec,&T::read_timeout_msec,&T::trigger_mask,&T::len);
   }
 };
...
struct ReadInput
 {
  ...
  static constexpr auto Fold()
   {
    return Ret(&T::con_id,&T::number,&T::len);
   }
 };
