...
inline PtrLen<const WChar> ZRange(const WChar *ztext) { return Range(ztext,ZLen(ztext)); }
...
bool FeedUnicode(PtrLen<const WChar> text,FuncType<bool,Unicode> auto func)
 {
  while( +text )
    {
...
void FeedUnicode(PtrLen<const WChar> text,FuncArgType<Unicode> auto func)
 {
  while( +text )
    {
     Unicode sym=CutUnicode(text);
...
class WCharToUtf8Full : NoCopy
 {
  ...
  public:
   ...
   explicit WCharToUtf8Full(const WChar *ztext) : WCharToUtf8Full(ZRange(ztext)) {}
