...
ErrorType UDPSocket::Outbound(Type sockid,Net::UDPoint dst,PtrLen<const uint8> data) noexcept
 {
  ...
  auto result=WinNN::sendto(sockid,data.ptr,data.len,0,address.getPtr(),address.getLen());
...
auto UDPSocket::Inbound(Type sockid,PtrLen<uint8> buf) noexcept -> InResult
 {
  ...
  Address address;
  WinNN::ulen_t address_len=address.getLen();

  auto result=WinNN::recvfrom(sockid,buf.ptr,buf.len,0,address.getPtr(),&address_len);
  ...
  if( WinNN::WSASendTo(sockid,&async->buf,1,&ret_len,0,address.getPtr(),address.getLen(),&async->olap,0) )
    {
     ...
     if( error==(Sys::ErrorType)WinNN::WSAErrorIOPending )
       {
        ret.pending=true;
        ret.error=NoError;
       }
